...
function PopupWithForm({ name, title, children, btnText, isOpen, onClose, onSubmit, isSbmtBtnActiv }) {
  return (
    <section className={`popup popup_type_${name} ${isOpen ? 'popup_opened' : ''}`}>
      <form name={name} method="POST" onSubmit={onSubmit} action="#" className={`popup__form popup__container popup__container_formtype_${name}`} noValidate>
        ...
        {children}
        <button type="submit" className={`popup__submit popup__btn popup__btn_action_${name} ${isSbmtBtnActiv ? '' : 'popup__btn_disabled'}`}>{btnText}</button>
